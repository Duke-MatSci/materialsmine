...
describe('Curate.vue', () => {
  ...
  it('renders curate category headers', () => {
    ...
    expect(catHeaders.at(2).text()).toBe('Valid Curation List Entry');
  });

  it('renders valid curation list Item', () => {
    expect.assertions(5);
    const section = wrapper
      .findAll('div > .section_teams > .curate > div')
      .at(1);
    const sectionItem = section.findAll('.md-layout-item.md-layout-item_card');
    expect(
      section.find('h2.visualize_header-h1.metamine_footer-ref-header').text()
    ).toEqual('Create Visualization');
    expect(
      section.find('.md-layout.md-layout-responsive').exists()
    ).toBeTruthy();
    expect(sectionItem.length).toBe(1);
    expect(sectionItem.at(0).attributes().class).toEqual(
      'md-layout-item md-layout-item_card'
    );
    expect(sectionItem.at(0).findComponent(RouterLinkStub).props().to).toEqual(
      '/explorer/chart/editor/new'
    );
  });
});
