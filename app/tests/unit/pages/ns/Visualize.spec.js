...
const totalNodeLength = () => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let childCount = element?.['subClasses'].length ?? 0; // Get the length of subClasses for each element
...
describe('Visualize.vue', () => {
  let wrapper;
  let mountMethod = jest.spyOn(Visualize.methods, 'formatClassData');
  let createNode = jest.spyOn(Visualize.methods, 'createNode');
  beforeEach(() => {
  ...
  describe('Visualize.vue without class Data', () => {
  ...
  describe('Visualize.vue with classes Data', () => {
    let createNodeObj = jest.spyOn(Visualize.methods, 'createNodeObj');
    let createLineObj = jest.spyOn(Visualize.methods, 'createLineObj');
