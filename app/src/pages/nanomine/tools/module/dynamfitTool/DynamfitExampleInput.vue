<template>
  <div class="main">
    <div class="dynamfitExampleInput">
      <h1 v-if="this.$store.state.versionNew" class="header-mm">Dynamfit - Prony Series coefficient fitting</h1>
      <v-container>
        <h1 v-if="this.$store.state.versionNew" class="header-nm">Dynamfit - Prony Series coefficient fitting</h1>
        ...
        <v-layout row wrap>
          <v-flex text-xs-left offset-xs0 offset-md0 offset-lg0>
            <v-btn color="primary" dark to="/DynamfitExample">
              <v-icon dark left>arrow_back</v-icon>Part I
            </v-btn>
          </v-flex>
        </v-layout>
        <br>
...
export default {
  ...
  created () {
    this.$store.commit('setAppHeaderInfo', { icon: 'workspaces', name: 'Dynamfit - Example' })
  }
}
</script>
