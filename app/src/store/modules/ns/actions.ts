export default {
  async fetchNsData(
    ...
    if (
      ...
      try {
        ...
        if (req.status !== 200) {
          throw new Error(req.statusText || 'Server error, cannot fetch namespace error');
        }
  ...
  async searchNSData(
    { getters, commit }: any,
    { query = '', singleResult = true }: { query?: string; singleResult?: boolean } = {}
  ) {
    ...
    const searchArray = (arr: any[], query: string, exactMatch = false) => {
      let result: any[] = [];
      ...
      for (let i = 0; i < arr.length; i++) {
        const element = arr[i];
        const elementID = element.ID.split('/').pop().split('#').pop().toLowerCase();
        ...
        if (element.subClasses && element.subClasses.length) {
          const foundInSub = searchArray(element.subClasses, query, exactMatch) as any;
    ...
    const isError = (singleResult && !response) || (!singleResult && !response.length);
