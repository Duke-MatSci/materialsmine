...
const deploymentTypes: Record<
  string,
  { deployUrl: string; statusUrl: string }
> = {
  general: {
...
function handleError({ commit, getters }: any) {
...
export default {
  async fetchVersions(context: any) {
  ...
  async deploy({ commit, rootGetters, getters, dispatch }: any, type: string) {
  ...
  async deploymentStatus(
    { commit, dispatch, rootGetters, getters }: any,
    type: string
  ) {
    const token = rootGetters['auth/token'];
    try {
  ...
  async fetchJsonSchema({ commit, rootGetters }: any) {
    const token = rootGetters['auth/token'];
    try {
      ...
      if (!response || response.status !== 201) {
        return commit(
          'setSnackbar',
          {
            message: (response as any)?.message || 'Something went wrong',
            duration: 7000,
          },
          { root: true }
        );
  ...
  async downloadJsonSchema({ commit, dispatch, rootGetters }: any) {
    const token = rootGetters['auth/token'];
    commit(
      'setSnackbar',
      {
  ...
  async publishSchema({ commit, dispatch, rootGetters }: any) {
