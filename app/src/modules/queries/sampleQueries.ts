interface SampleQueries {
  header: (sampleId: string) => string;
  curatedProperties: (sampleId: string) => string;
  sampleImages: (sampleId: string) => string;
  materialComponents: (sampleId: string) => string;
  processingSteps: (sampleId: string) => string;
  otherSamples: (sampleId: string) => string;
  processLabel: (sampleId: string) => string;
  facetFilterMaterial: () => string;
  getSearchFacetFilterMaterialCount: (label: string) => string;
  getSearchFacetFilterMaterialDefinition: (label: string) => string;
  getSearchFacetFilterMaterial: (label: string) => string;
}
...
const sampleQueries: SampleQueries = {
  header: (
    sampleId: string
  ): string => `${commonPrefixes}SELECT DISTINCT (REPLACE(STR(?doi),"http://dx.doi.org/","") AS ?DOI) ?sample_label ?process_label WHERE {
  ...
  curatedProperties: (
    sampleId: string
  ): string => `${commonPrefixes}SELECT DISTINCT ?AttrType ?value (GROUP_CONCAT(DISTINCT ?unit; SEPARATOR="; ") AS ?Units) WHERE {
    VALUES ?sample { <http://materialsmine.org/sample/${sampleId}> }
      ?sample a mm:PolymerNanocomposite ;
  ...
  sampleImages: (sampleId: string): string => `${commonPrefixes}SELECT DISTINCT * WHERE {
    VALUES ?sample { <http://materialsmine.org/sample/${sampleId}>}
    ?sample a mm:PolymerNanocomposite ;
  ...
  materialComponents: (
    sampleId: string
  ): string => `${commonPrefixes}SELECT DISTINCT ?std_name ?role ?attrType ?attrValue (GROUP_CONCAT(DISTINCT ?AttrUnit; SEPARATOR="; ") AS ?attrUnits) ?fullLabel WHERE {
    VALUES ?sample {<http://materialsmine.org/sample/${sampleId}>}
    {
      ?sample a mm:PolymerNanocomposite ;
  ...
  processingSteps: (
    sampleId: string
  ): string => `${commonPrefixes}SELECT ?step ?param_label (GROUP_CONCAT(?ParamDescr; SEPARATOR="; ") AS ?Descr)
  WHERE {
    {
  ...
  otherSamples: (sampleId: string): string => `${commonPrefixes}SELECT DISTINCT ?sample WHERE {
  ...
  processLabel: (sampleId: string): string => `${commonPrefixes}
  ...
  facetFilterMaterial: (): string => `PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
  PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
  ...
  getSearchFacetFilterMaterialCount: (
    label: string
  ): string => `PREFIX sio: <http://semanticscience.org/resource/>
  PREFIX nm: <http://materialsmine.org/ns/>
  PREFIX dct: <http://purl.org/dc/terms/>
  ...
  getSearchFacetFilterMaterialDefinition: (
    label: string
  ): string => `PREFIX skos: <http://www.w3.org/2004/02/skos/core#>
  PREFIX nm: <http://materialsmine.org/ns/>
  SELECT DISTINCT * WHERE {
    nm:${label} skos:definition ?definition
  }`,

  getSearchFacetFilterMaterial: (
    label: string
  ): string => `PREFIX sio: <http://semanticscience.org/resource/>
  PREFIX nm: <http://materialsmine.org/ns/>
  PREFIX dct: <http://purl.org/dc/terms/>
