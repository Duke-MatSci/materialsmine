...
<div class="md-card-actions viz-u-display__show">
  <template v-if="(typeof inputObj === 'object')">
    ...
    <md-field :class="[inputError ? 'md-invalid' : '']" v-else-if="inputObj.type === 'List'" :style="reduceSpacing" >
      <md-select v-model="inputObj.cellValue"  :name="uniqueKey.join(',')" :id="uniqueKey.join(',')" @md-opened="fetchValues"
      :placeholder="`Please choose ${name}`" :md-dense="true" md-align-trigger>
    ...
    <div v-else-if="inputObj.type === 'File'" >
      <div v-if="!!inputObj.cellValue" class="md-layout md-alignment-center-space-between">
        <p class="md-body-2" :class="[fileError ? 'u--color-error' : '']">{{ reduceCellValue }}.....</p>

        <md-button class="md-icon-button" @click="downloadImage(inputObj.cellValue)">
          <md-icon>download</md-icon> <md-tooltip md-direction="top">Click to download file</md-tooltip>
        </md-button>
        <md-button class="md-icon-button" @click="confirmDelete">
          <md-icon>close</md-icon> <md-tooltip md-direction="top">Click to remove file</md-tooltip>
        </md-button>
      </div>
      <label v-else :for="uniqueKey.join(',')">
        <div class="form__file-input">
          <div :style="reduceSpacing"
          :class="[fileError && !inputObj.cellValuecellValue ? 'md-invalid' : '', inputObj.required ? 'md-required md-has-file' : '', 'md-field md-theme-default']">
            <md-icon>attach_file</md-icon>
            <label style="top: 4px;" :for="uniqueKey.join(',')">Select {{name}} to upload</label>
            <div class="md-file">
              <input @change="confirmUpload" type="file" :name="uniqueKey.join(',')" :id="uniqueKey.join(',')" />
            </div>
            <span class="md-error" style="margin-left:40px">At least one file is required</span>
          </div>
          <md-tooltip md-direction="top">{{ parent }}</md-tooltip>
        </div>
      </label>
    ...
    <div v-else>Input Type <span class="md-title">'{{ inputObj.type }}'</span> not valid</div>
...
export default {
  ...
  computed: {
      ...
      const refData = this.uniqueKey.reduce(function (o, x, idx, arr) {
        ...
        if (Array.isArray(o[x])) {
          for (let i = 0; i < o[x].length; i++) {
            if (Object.hasOwnProperty.call(o[x][i], arr[idx + 1])) { return o[x][i] }
          }
        }
        return o[x]
      }, obj)
    ...
    stringInputVal () {
      ...
      return Object.hasOwnProperty.call(this.inputObj, 'cellValue') &&
      this.inputObj.cellValue !== null && this.inputObj.cellValue.trim() === ''
    }
  },
  watch: {
    stringInputVal (newVal) {
      if (newVal === true) { this.inputObj.cellValue = null }
    }
  },
  methods: {
    ...mapActions({
    ...
    async fetchValues () {
      ...
      this.error = false
      try {
        const result = await this.fetchXlsList({ field: this.inputObj.validList })
    ...
    confirmDelete () {
      ...
      const msg = 'Removing this file would clear all fields in this block. Please confirm your action'
