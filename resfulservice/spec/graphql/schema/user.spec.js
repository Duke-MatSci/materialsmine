...
describe('Schema Unit Tests:', function () {
  ...
  it('should have _id,alias,givenName,surName,displayName,email,apiAccess as fields for User schema', async function () {
    const user = graphQlSchema.getType('User');
    const keys = Object.keys(user.getFields());
    expect(keys).to.include.members(['_id', 'alias', 'givenName', 'surName', 'displayName', 'email', 'apiAccess']);
  });

  it('should have correct datatypes for User schema', async function () {
    const user = graphQlSchema.getType('User');
    const { _id, alias, givenName, surName, displayName, email, apiAccess } = user.getFields();
